...
def health_check():
    ...
    return {"status": "healthy", "corpus_size": len(retriever.corpus), "index_loaded": retriever.index is not None}
...
def retrieve_endpoint(request: QueryRequest):
    ...
    if not request.top_k:
        request.top_k = config.retrieval_topk  # fallback to default
    ...
    results = results[0]
    scores = scores[0]
    formatted_results = [{"id": f"doc_{i}", "content": result['contents'], "score": scores[i-1]} for i, result in enumerate(results, 1)]
    # Format response
    return {"query": request.query, "method": "dense", "results": formatted_results, "num_results": len(formatted_results)}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--index_path", type=str, default="data/e5_Flat.index", help="Corpus indexing file."
    )
    parser.add_argument(
        "--corpus_path",
        type=str,
