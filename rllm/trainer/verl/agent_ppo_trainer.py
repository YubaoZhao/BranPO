...
class AgentPPOTrainer(RayPPOTrainer):
    ...
    def _validate_agent(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if self.config.rllm.stepwise_advantage.enable:
                ...
                test_output_gen_batch, _ = self.generate_agent_trajectory(meta_info=test_batch.meta_info, save_dir_name="val_chat_completions")
    ...
    def generate_agent_trajectory(self, timing_raw=None, meta_info=None, save_dir_name=None):
        """
    ...
    def _transform_agent_trajectories(self, trajectories: list[dict], save_dir_name: str=None):
        ...
        for traj in trajectories:
            ...
            all_logprobs_list.append(traj["rollout_log_probs"])
        ...
        open_mode = 'w'
        if save_dir_name != "chat_completions":
            open_mode = 'a+'
        with open(os.path.join(save_dir, f"{self.global_steps}.jsonl"), open_mode) as f:
            for traj in trajectories:
                f.write(json.dumps({"messages": traj['chat_completions'], "task": traj['task']}) + "\n")
