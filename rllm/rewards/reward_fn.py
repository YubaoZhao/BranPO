...
from rllm.rewards.search_reward import RewardSearchFn, RewardSearchR1Fn
...
def search_r1_reward_fn(task_info: dict, action: str) -> RewardOutput:
    """
    A reward function for search tasks that implements the RewardFunction protocol.
    ...
    """
    reward_config = RewardConfig()
    reward_fn = RewardSearchR1Fn(reward_config)
    if isinstance(action, Action):
        action = action.action
